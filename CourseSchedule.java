...
public class CourseSchedule {
    public static void main(String[] args) {
        ...
        int[] order = tester.findOrder(n, prerequisites);
    ...
    public int[] findOrder(int n, int[][] prerequisites) {
        ...
        for (int i=0; i<n; i++) {
            if (!study(graph, i, order, finished, visited)) {
                return new int[0];
            }
        }
    ...
    private boolean study(Map<Integer, List<Integer>> graph, int course, List<Integer> order, Set<Integer> finished,
            Set<Integer> visited) {
        if (finished.contains(course)) return true;
        if (visited.contains(course) && !finished.contains(course)) return false;

        visited.add(course);
        for (Integer prerequisite:graph.get(course)) {
            if (!finished.contains(prerequisite)) {
                if (!study(graph, prerequisite, order, finished, visited)) {
                    return false;
                }
            }
        }
